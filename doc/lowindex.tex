...
\begin{thm}[\cite{Massey} Theorem 4.2]
  ...
  \item Construct a CW-complex $X$ with base vertex $x$ such that
   ...
   A {\it covering subgraph} $C$ of $B$ with degree $d$ is a topological space
   constructed from a degree $d$ covering map $p:\tilde B \to B$ where the edges
   of $\tilde B$ are oriented and labeled by lifting the labels and orientations
   from $B$.  The space $C$ is given as $\tilde B - P$ where $P$ is some subset
   of $p^{-1}(\{p_1, \ldots, p_r\})$.  The vertices of $C$ are labeled by integers
   in $\{1, \ldots, d}$.  Two covering subgraphs are isomorphic if there is a
   homeomorphism between them which preserverses labels of vertices and edges
   and preserves the orientations of the edges.
